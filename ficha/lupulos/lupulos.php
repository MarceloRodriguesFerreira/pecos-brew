...
if (isset($_POST['salvar'])) {
    ...
    if ($edit_id > 0) {
        $stmt = $conn->prepare("UPDATE ingredientes_lupulo SET nome=?, idx_alfa_acido=?, quantidade=?, tempo_adicao=? WHERE id=?");
        ...
        $stmt->bind_param("sddsi", $nome, $idx_alfa_acido, $quantidade, $tempo_adicao, $edit_id);
    } else {
        $stmt = $conn->prepare("INSERT INTO ingredientes_lupulo (ficha_id, nome, idx_alfa_acido, quantidade, tempo_adicao) VALUES (?, ?, ?, ?, ?)");
...
<div class="container py-5">
    ...
    <form method="post">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-4">
                <label>Tempo de Adição</label>
                <input type="text" name="tempo_adicao" class="form-control" value="<?= htmlspecialchars($tempo_adicao) ?>" required placeholder="Ex: 60 min, Whirlpool, Dry Hop">
            </div>
        </div>
