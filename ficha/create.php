...
if ($_POST) {
    $stmt = $conn->prepare(
        "INSERT INTO ficha_brassagem (nome_receita, estilo, numero_lote, data_brassagem, cervejeiro_responsavel, og, fg, ibu, ebc, eficiencia, volume_inicial, volume_final, tempo_fervura, temperatura_fermentacao, tempo_fermentacao, temperatura_maturacao, tempo_maturacao, tipo_envase, volume_envase, carbonatacao, observacoes) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
    $stmt->bind_param(
        "ssssssssdddddddiddsss",
